...
document.addEventListener("DOMContentLoaded", function() {
    ...
    document.getElementById("download"))
        try {
            ...
            var file_name = "downloaded.txt"; // Default file name for plain texta
...
function _() {
    var _ = content.value
      , $ = ""
      , ee = function(_) {
        for (var [$,ee,te,_="string"] = [_, ae, !1], ne = [], c = 0; c < $.length; c++) {
            var oe = $.charAt(c)
              , re = oe;
            ee[re = te ? oe.toLowerCase() : re] ? ne.push(ee[re]) : ne.push(oe)
        }
        return ne = "string" == _ ? (ne = ne.join("")).replace(/\n/g, "<br>") : ne
